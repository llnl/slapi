...
sub parse_args
{
    ...
    # Try to get the version information from the MODULEVERSION and MODULERELEASE files
    if (!$packageversion)
    {
        $packageversion = `cat MODULEVERSION`; chomp($packageversion);
    }
    if (!$packagerelease)
    {
        $packagerelease = `cat MODULERELEASE`; chomp($packagerelease);
    }
    if (!$pythonversion)
    {
    ...
    if ($packagename =~ m#python-# && $pythonversion)
    {
        $fullpackagename =~ s/python-/python$pythonversion-/;
    }
...
sub build_tarball
{
    ...
    if ($scmtype eq "git")
    {
        ...
        $savedir = getcwd();
        chdir($topdir);
        $cmd = "$git clone --branch ${fullpackagename}-${packageversion}-${packagerelease} ${scmurl} ${packagename}-${packageversion}";
        print("$cmd\n");
        $rc = system("$cmd");
        $rc = $rc >> 8;
        if ($rc != 0)
