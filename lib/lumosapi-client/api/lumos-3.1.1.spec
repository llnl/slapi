...
tags:
  ...
  - name: Boa
    description: Supported on Boa libraries.
paths:
  /spec:
    get:
  ...
  /backups:
    get:
      ...
      description: |- 
        Returns a paginated list of both manually and automatically generated backups currently stored on the library 
        in order of newest to oldest. Backups stored on a USB device connected to the LS will be included in the list. USB 
        backup files must be located in the /backups directory of a USB device to be detected.
  ...
  '/backups/{name}':
    ...
    delete:
      ...
      description: Deletes a backup stored on the internal library storage. This will not delete backups from any connected USB devices.
  ...
  '/backups/{name}/download':
    ...
    get:
      ...
      responses:
        ...
        default:
          $ref: '#/components/responses/default'
  /backups/{name}/save-to-usb:
    parameters:
      - $ref: '#/components/parameters/backupName'
    post:
      summary: Save a Backup to USB
      description: |-
        Save the specified backup file to all USBs connected to the LS. It will be saved to the /backups directory on each USB.
        If the /backups directory does not exist, it will be created.
      operationId: SaveBackupToUSB
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Admin, SuperUser ]
      responses:
        '204':
          ...
          application/json:
            schema:
              ...
              properties:
                ...
                readOnly:
                  type: boolean
                  description: Indicates if the server should only be used for read operations (e.g. locate or get KMIP
                    operations). This should be set to true for any server that is configured as a backup or clone and
                    only supports retrieval of keys.
                  default: false
              required:
                - address
                - port
  ...
  /library/demo:
    post:
      ...
      description: |-
        Start demo mode on the library for the specified amount of time, or until an abort is received.
        Demo mode cannot be run on partitions with SlotIQ enabled or on cleaning partitions.
      operationId: StartDemoMode
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser ]
      x-experimental: true
      requestBody:
        ...
        content:
          application/json:
            schema:
              ...
              properties:
                duration:
                  description: Duration of demo mode in seconds
                  type: integer
                  format: int64
                  minimum: 1
                  maximum: 28800
                  example: 3600
                partition:
                  description: Storage partition on which to run demo mode
                  type: string
      ...
      responses:
        ...
        default:
          $ref: '#/components/responses/default'
  /library/demo/abort/{taskID}:
    put:
      summary: Stop Demo Mode
      description: Abort demo mode. Any moves initiated by demo mode that are already in progress will continue to run after demo mode has been aborted.
      operationId: StopDemoMode
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser ]
      x-experimental: true
      parameters:
  ...
  /library/diagnostics/delete-geometry:
    post:
      ...
      description: |-
        ...
        the library. The inventory of the library will be preserved during this operation but may be rediscovered
        if new magazines are found, magazines have been moved, or if frames have been added to or removed from the library. NOTE - Remove all tapes from tape
  ...
  /library/environment:
    get:
      summary: Retrieve Library Environment
      description: Retrieve the current temperature and humidity of the library from its sensors.
      operationId: GetLibraryEnvironment
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser ]
      responses:
        '200':
  ...
  /logs/download:
    get:
      ...
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Admin, SuperUser, Degraded ]
      parameters:
        ...
        - description: |-
            Save the gathered logs to all USBs connected to the LS. They will be saved to the /logs directory on each USB.
            If the /logs directory does not exist, it will be created.
          name: saveToUSBs
          in: query
          schema:
            type: boolean
        - name: uploadToSupport
          schema:
            type: boolean
          in: query
          description: |-
            Upload the gathered logs to Spectra Logic Support. If not supplied, defaults to false.
  ...
  '/mlm-dlm/drive-test':
    post:
      summary: Start a Drive Test
      description: |-
        Cleans the drive and performs a Post-B test to verify its write capability. The drive must be in a storage
        partition with an associated cleaning partition that contains at least one cleaning tape with remaining cleans
        and at least one Spectra Diagnostic Cartridge of matching generation with remaining Drive Tests.
      operationId: StartDriveTest
      tags: [ Cube, TFinity, Python, Boa ]
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/DriveTestRequest'
        required: true
      x-permitted-roles: [ Admin, SuperUser ]
      responses:
        '202':
        ...
        default:
          $ref: '#/components/responses/default'
    get:
      summary: Retrieve Drive Test Information
      description: Retrieve the status and results of previously run Drive Tests.
      operationId: GetDriveTestResults
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Admin, Operator, SuperUser ]
      parameters:
        - name: offset
        ...
        - name: limit
          ...
          schema:
            ...
            default: 100
            example: 10
        - name: driveManufacturerSerialNumber
          in: query
          description: Filter results to only include drive tests for the specified drive manufacturer serial number.
          schema:
            type: string
            example: "10WT046832"
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/DriveTestList'
        default:
          $ref: '#/components/responses/default'
  '/mlm-dlm/drive-test/{taskID}':
    get:
      summary: Retrieve Results for a Drive Test
      description: Retrieve the status and results of a previously run Drive Test.
      parameters:
        - $ref: '#/components/parameters/taskID'
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Admin, Operator, SuperUser ]
      operationId: GetDriveTestResult
      responses:
        '200':
          description: OK
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/DriveTest'
        default:
          $ref: '#/components/responses/default'
  '/mlm-dlm/media-test':
    post:
      summary: Start a Media Test
      description: |-
        Perform a read verification test on the specified tape media using the specified drive. This will verify the data
        on the first wrap of the tape. The drive and tape must be in the same partition and of compatible generations. The tape
        will be returned to its original location after the test completes.
      operationId: StartMediaTest
      tags: [ Cube, TFinity, Python, Boa ]
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/MediaTestRequest'
        required: true
      x-permitted-roles: [ Admin, SuperUser ]
      responses:
        '202':
        ...
        default:
          $ref: '#/components/responses/default'
    get:
      summary: Retrieve Media Test Information
      description: Retrieve the status and results of currently running and previously run Media Tests.
      operationId: GetMediaTestResults
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Admin, Operator, SuperUser ]
      parameters:
        - name: offset
        ...
        - name: tapeSerialNumber
          in: query
          description: Filter results to only include media tests for the specified tape serial number.
          schema:
            type: string
            example: "AD5KR89X6H"
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/MediaTestList'
        default:
          $ref: '#/components/responses/default'
  /mlm-dlm/media-test/{taskID}:
    get:
      summary: Retrieve Results for a Media Test
      description: Retrieve the status and results of a specific Media Test.
      parameters:
        - $ref: '#/components/parameters/taskID'
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Admin, Operator, SuperUser ]
      operationId: GetMediaTestResult
      responses:
        '200':
          description: OK
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/MediaTest'
        default:
          $ref: '#/components/responses/default'
  /mlm-dlm/mounts:
    get:
      summary: Retrieve mount records containing Media Lifecycle Management (MLM) and Drive Lifecycle Management (DLM)
        information.
      description: Retrieve a list of mount records containing Media Lifecycle Management (MLM) and Drive Lifecycle
        Management (DLM) information from the library. Results are returned in descending unload time (newest first) by
        default.
      operationId: GetMLMDLMMounts
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser ]
      x-experimental: true
      parameters:
        ...
        - name: after
          in: query
          description: The cursor to start retrieving mount records after. This should be set to the afterCursor value
            from a previous response which allows for forward pagination.
          schema:
            type: string
        - name: before
          in: query
          description: The cursor to start retrieving mount records before. This should be set to the beforeCursor value
            from a previous response which allows for backward pagination.
          schema:
            type: string
        - name: sortOrder
          in: query
          description: Specify the sort order for the returned records. This must remain consistent between the initial
            and all subsequent page requests.
          schema:
            $ref: '#/components/schemas/MLMDLMMountSortOrder'
            default: "UNLOAD_TIME_DESC"
        - name: driveManufacturerSerialNumber
          in: query
          description: Filter results to only include mount records for the specified drive manufacturer serial number.
          schema:
            type: string
            example: "10WT046832"
        - name: tapeSerialNumber
          in: query
          description: Filter results to only include mount records for the specified tape serial number.
          schema:
            type: string
            example: "AD5KR89X6H"
        - name: unloadTimeMin
          in: query
          description: Filter results to only include mount records on or after the specified unload time.
          schema:
            type: string
            format: date-time
        - name: unloadTimeMax
          in: query
          description: Filter results to only include mount records on or before the specified unload time.
          schema:
            type: string
            format: date-time
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/MLMDLMMountList'
        default:
          $ref: '#/components/responses/default'
  /mlm-dlm/mounts/most-recent-per-tape:
    get:
      summary: Retrieve the most recent mount record for each tape containing Media Lifecycle Management (MLM) and
        Drive Lifecycle Management (DLM) information.
      description: Retrieve the most recent mount record for each tape containing Media Lifecycle Management (MLM)
        and Drive Lifecycle Management (DLM) information from the library.
      operationId: GetMostRecentMLMDLMMountPerTape
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser ]
      x-experimental: true
      parameters:
        - name: limit
          ...
          schema:
            ...
            example: 10
        - name: offset
          description: The number of items to skip before starting to collect the result set. If not included, no items
            are skipped.
          in: query
          schema:
            type: integer
        ...
        - name: present
          in: query
          description: Filter results to only include mount records for tapes that are currently present in the library.
          schema:
            type: boolean
            default: true
        - name: tapeSerialNumber
          in: query
          description: Filter results to include only mount records whose tape serial number partially or fully matches the value provided.
                      For example, querying for 'AD5' will return any record with a serial number that contains 'AD5' anywhere within it.
          schema:
            type: string
            example: "AD5"
        - name: barcode
          in: query
          description: Filter results to include only mount records whose barcode partially or fully matches the value provided.
                      For example, querying for 'L8' will return any record with a barcode that contains 'L8' anywhere within it.
          schema:
            type: string
            example: "L8"
        - name: tapeHealthMin
          in: query
          description: Filter results to only include mount records where the tapeHealth is equal to or greater than the specified tapeHealth.
          schema:
            type: number
            format: double
        - name: tapeHealthMax
          in: query
          description: Filter results to only include mount records where the tapeHealth is equal to or less than the specified tapeHealth.
          schema:
            type: number
            format: double
        - name: manufactureDateMin
          in: query
          description: Filter results to only include mount records for tapes that were manufactured on or after the specified time.
          schema:
            type: string
            format: date-time
        - name: manufactureDateMax
          in: query
          description: Filter results to only include mount records for tapes that were manufactured on or before the specified time.
          schema:
            type: string
            format: date-time
        - name: remainingCapacityMBMin
          in: query
          description: |-
            Filter results to only include mount records where the remaining capacity is equal to or greater than the
            specified remaining capacity in megabytes.
          schema:
            type: integer
            format: int64
            minimum: 0
            example: 0
        - name: remainingCapacityMBMax
          in: query
          description: |-
            Filter results to only include mount records where the remaining capacity is equal to or less than the
            specified remaining capacity in megabytes.
          schema:
            type: integer
            format: int64
            minimum: 0
            example: 0
        - name: usedCapacityMBMin
          in: query
          description: |-
            Filter results to only include mount records where the used capacity is equal to or greater than the
            specified used capacity in megabytes.
          schema:
            type: integer
            format: int64
            minimum: 0
            example: 0
        - name: usedCapacityMBMax
          in: query
          description: |-
            Filter results to only include mount records where the used capacity is equal to or less than the specified
            used capacity in megabytes.
          schema:
            type: integer
            format: int64
            minimum: 0
            example: 0
        - name: libraryPartition
          in: query
          description: Filter results to include only mount records whose library partition partially or fully matches the value provided.
                      For example, querying for 'Data Partition' will return any record with a library partition that contains 'Data Partition' anywhere within it.
          schema:
            type: string
            example: "Data Partition"
        - name: tapeUsageCategory
          in: query
          description: Filter results to include only mount records whose tape usage category type matches the specified
              value.
          schema:
            $ref: '#/components/schemas/MLMDLMMountTapeUsageCategory'
        - name: spectraCertifiedMedia
          in: query
          description: Filter results to only include mount records for tapes that are or are not Spectra Certified Media.
              Not providing this query parameter will return mount records for both Spectra Certified Media and non-Spectra Certified Media.
          schema:
            type: boolean
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/MostRecentMLMDLMMountPerTapeList'
        default:
          $ref: '#/components/responses/default'
  /mlm-dlm/mounts/most-recent-per-drive:
    get:
      summary: Retrieve the most recent mount record for each drive containing Media Lifecycle Management (MLM) and
        Drive Lifecycle Management (DLM) information.
      description: Retrieve the most recent mount record for each drive containing Media Lifecycle Management (MLM)
        and Drive Lifecycle Management (DLM) information from the library.
      operationId: GetMostRecentMLMDLMMountPerDrive
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser ]
      x-experimental: true
      parameters:
      ...
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/MostRecentMLMDLMMountPerDriveList'
        default:
          $ref: '#/components/responses/default'
  /mlm-dlm/imports:
    get:
      summary: Retrieve tape imports
      description: Retrieve a list of tape import records
      operationId: GetTapeImports
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser ]
      x-experimental: true
      parameters:
        ...
        - name: tapeSerialNumber
          in: query
          description: Filter results to only include imports of the tape with the specified serial number.
          schema:
            type: string
            example: "AD5KR89X6H"
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/TapeImportList'
        default:
          $ref: '#/components/responses/default'
  /mlm-dlm/exports:
    get:
      summary: Retrieve tape exports
      description: Retrieve a list of tape export records
      operationId: GetTapeExports
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser ]
      x-experimental: true
      parameters:
        ...
        - name: tapeSerialNumber
          in: query
          description: Filter results to only include exports of the tape with the specified serial number.
          schema:
            type: string
            example: "AD5KR89X6H"
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/TapeExportList'
        default:
          $ref: '#/components/responses/default'
  /mlm-dlm/mounts/most-recent-per-tape/{tapeSerialNumber}:
    delete:
      summary: Archive Most Recent Mount Record for a Tape
      description: |-
        Archives the most recent mount record for the specified tape. After archiving, this tape will no longer appear in the “most recent mount per tape” response.
        If the tape is mounted and then unmounted again, a new mount record will be created and returned in the “most recent mount per tape” response.
        The archived mount record will remain available in the “most recent mount per drive” records and in the full mount record history.
      operationId: ArchiveMostRecentMountRecordForTape
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Admin, SuperUser ]
      parameters:
        - name: tapeSerialNumber
          in: path
          description: The serial number of the tape for which to archive.
          required: true
          schema:
            type: string
          example: "4191106010"
  ...
  /packages:
    get:
      ...
      description: |-
        ...
        directory on the USB device. The preferred location is the /packages directory of the USB device.

        For backward compatibility, the root directory of the USB device will also be scanned for packages; however,
        this behavior is deprecated and may be removed in a future release.
    ...
    put:
      ...
      responses:
        ...
        default:
          $ref: '#/components/responses/default'
    patch:
      summary: Update Library Syslog Settings
      description: Update the value of the library syslog settings.
      operationId: UpdateSyslogSettings
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Admin, SuperUser ]
      requestBody:
        required: true
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/UpdateSyslogSettingsRequest'
      responses:
        '204':
          $ref: '#/components/responses/204'
        default:
  ...
  /summary/environment:
    get:
      summary: Get a summary of temperature and humidity within the library
      description: |-
        Get the most recent historical aggregates of the temperature and humidity from sensors in the library
  ...
  /auth/login:
    post:
      ...
      description: >
        Authenticates the user using the given credentials and returns a token accepted by the rest of the API.
        Set-Cookie headers are provided for web clients to store "Authorization" and "Authorization-Metadata" cookies.
        "Authorization" contains the token, while "Authorization-Metadata" contains a URL-encoded JSON object.

        If LumOS user authentication is degraded, due to e.g. a critical database error during startup,
        the returned token will be for a user that belongs to the "Degraded" group with extremely limited access to the
        rest of this API. (Note that you cannot add regular users to this group.)
      operationId: Login
      tags: [ Cube, TFinity, Python, Boa ]
      security: [ ]
      requestBody:
  ...
  /auth/refresh:
    post:
      ...
      tags: [ Cube, TFinity, Python, Boa ]
      x-permitted-roles: [ Operator, Admin, SuperUser, Degraded ]
      responses:
        '200':
          description: OK
          headers:
...
components:
  parameters:
    ...
    barcodeParam:
      ...
      schema:
        ...
        example: "000797L6"
    containerType:
      description: The type of media container. Type "ALL" is deprecated. To filter for all container types exclude the query parameter.
        <table>
        <tr>
        <th><b>Type</b></th>
        <th>Description</th>
        ...
        <tr>
        <td>IE_SLOT</td>
        <td>A slot in a TeraPack magazine that was assigned to an entry/exit chamber</td>
        </tr>
        <tr>
        <td>DRIVE</td>
  ...
  schemas:
      ...
      allOf:
        - type: object
          ...
          properties:
            ...
            savedOnLS:
              type: boolean
              description: Whether the backup file is currently stored on the library. This will be true if the backup is saved in the library's internal storage.
            savedOnUSB:
              type: boolean
              description: Whether the backup file is currently stored on a connected USB drive. This will be true if the backup is saved on any USB drive that is connected to the LS.
      example:
    ...
    BackupSettings:
      ...
      properties:
        ...
        autoSaveToUSB:
          type: boolean
          description: |-
            Enable or disable automatic saving of backups to connected USB drives. When enabled, newly created manual and
            automatic backups will be copied to the /backups directory on all USB drives connected to the LS.
            If the /backups directory does not exist on the USB drive, it will be created.
      required:
        - enabled
        - frequencyInSeconds
        - autoSaveToUSB
    ...
    ManualTime:
      description: |-
        Time to set the library to, specified in RFC 3339 format
      type: string
      format: date-time
    NTPServers:
      description: |-
        List of NTP servers to use for time synchronization. The servers may be hostnames
    ...
    ContainerTypes:
      description: |-
        ContainerTypes of ALL is deprecated.
        ...
        - type: object
          ...
          properties:
            ...
            portConnectionStatuses:
              $ref: "#/components/schemas/PortConnectionStatuses"
    PortConnectionStatuses:
      title: PortConnectionStatuses
      description: |-
        Connection status of the drive's fibre channel or SAS ports.
      type: object
      properties:
        portA:
          $ref: "#/components/schemas/PortConnectionStatus"
        portB:
          $ref: "#/components/schemas/PortConnectionStatus"
    PortConnectionStatus:
      title: PortConnectionStatus
      description: |-
        Connection status of a drive fibre channel or SAS port.
      type: object
      oneOf:
        - $ref: "#/components/schemas/SASPortConnectionStatus"
        - $ref: "#/components/schemas/FibreChannelPortConnectionStatus"
      discriminator:
        propertyName: portType
        mapping:
          SAS: '#/components/schemas/SASPortConnectionStatus'
          FIBRE_CHANNEL: '#/components/schemas/FibreChannelPortConnectionStatus'
    PortConnectionPortType:
      description: Type of the drive port
      type: string
      enum:
        - "SAS"
        - "FIBRE_CHANNEL"
    SASPortConnectionStatus:
      title: SASPortConnectionStatus
      description: |-
        Connection status of a drive SAS port.
      type: object
      required:
        - portType
      properties:
        portType:
          $ref: "#/components/schemas/PortConnectionPortType"
        physicalLinkRate:
          $ref: "#/components/schemas/SASPhysicalLinkRate"
        signalDetected:
          description: Indicates if signal (i.e. light) is detected on this port
          type: boolean
        portInitializationComplete:
          description: Indicates if the port initialization process has completed
          type: boolean
        hashedSASAddress:
          type: string
          description:
            The 24 bit hashed address that is assigned to the port. Based on the WWPN
        sasAddress:
          type: string
          description:
            The SAS address of the port. i.e. the WWPN
    FibreChannelPortConnectionStatus:
      title: Fibre Channel Port Connection Status
      description: |-
        Connection status of a drive Fibre Channel port.
      type: object
      required:
        - portType
      properties:
        portType:
          $ref: "#/components/schemas/PortConnectionPortType"
        currentTopology:
          $ref: "#/components/schemas/PortTopology"
        currentSpeed:
          $ref: "#/components/schemas/FibreChannelSpeed"
        loginComplete:
          description: Indicates if at least one host is currently logged in to the drive on this port
          type: boolean
        conflictDetected:
          description: Indicates if the required Hard Arbitrated Loop Physical Address is in use by another device
            or no Arbitrated Loop Physical Address is available for this port.
          type: boolean
        signalDetected:
          description: Indicates if signal (i.e. light) is detected on this port
          type: boolean
        portInitializationComplete:
          ...
          type: boolean
        currentNPortID:
          description: The 24-bit N_Port_ID currently assigned to this port. Only set when the portInitializationComplete field is true.
          type: integer
        currentFibreChannelArbitratedLoopLoopID:
          description: The 6-bit Fibre Channel Arbitrated Loop Loop ID currently assigned to this port. Only set when the portInitializationComplete field is true and the currentTopology is LOOP.
          type: integer
        currentPortName:
          description: The port's name identifier (i.e. WWPN)
          type: string
        currentNodeName:
          description: The node's name identifier (i.e. WWNN)
          type: string
    SASPhysicalLinkRate:
      title: SASPhysicalLinkRate
      description: |-
        Speed or connection of the SAS port. This indicates either the negotiated link rate or the reason
      ...
      enum:
        ...
        - "12Gbps"
    FibreChannelSpeed:
      title: FibreChannelSpeed
      description: |-
        Speed of drive fibre channel port.
      type: string
      enum:
        - "UNKNOWN"
        - "1Gbps"
    ...
    PortTopology:
      title: PortTopology
      description: |-
        Topology type for drive fibre channel port.
      type: string
      enum:
        - "POINT_TO_POINT"
        - "LOOP"
    ...
    FRU:
      ...
      discriminator:
        ...
        mapping:
          ...
          FRONT_PANEL: '#/components/schemas/GenericFRU'
    GenericFRU:
    ...
    FRUBase:
      ...
      properties:
        ...
        type:
          ...
          $ref: '#/components/schemas/FRUTypes'
        fruFirmware:
          description: FRU firmware version. Set to empty string for FRUs with no/non-updatable firmware, e.g. power supplies
          type: string
        actions:
    ...
    ReadElementStatusInformation:
      description: |-
        ...
        <table>
          ...
          <tr>
          <td>MEDIA_ZONING</td>
          <td>Include zone information and TeraPack barcodes in Read Element Status (RES) responses. Media Zoning is only supported on TFinity libraries.</td>
          </tr>
        </table>
      type: string
      enum:
    ...
    SyslogSettingsResponse:
      ...
      properties:
        ...
        port:
          ...
          default: 514
        network:
          $ref: '#/components/schemas/SyslogNetwork'
          default: 'UDP'
    SyslogNetwork:
      description: Network protocol used for syslog connections.
      type: string
      enum:
        - "UDP"
        - "TCP"
    UpdateSyslogSettingsRequest:
      title: Update Syslog Settings Request
      type: object
      properties:
        remoteServer:
          type: string
          description: The IP address or hostname of the remote syslog server. Remote servers are not validated for reachability. Use an empty name to disable syslog.
        port:
          type: integer
          description: Port of the remote syslog server.
          minimum: 1
    ...
    UpdateEnvironmentSettingsRequest:
      ...
      properties:
        ...
        robot1TemperatureOffset:
          type: number
          format: double
          description: Temperature offset in degrees Celsius for Robot:1. This value is added to the temperature reported by the sensor on the HPT to calculate the actual temperature near the robot. By default, this is set to 0. This must be set if robot1HumidityOffset is set.
        robot1HumidityOffset:
          type: number
          format: double
          description: Humidity offset in percent relative humidity for Robot:1. This value is added to the humidity reported by the sensor on the HPT to calculate the actual humidity near the robot. By default, this is set to 0. This must be set if robot1TemperatureOffset is set.
        robot2TemperatureOffset:
          type: number
          format: double
          description: Temperature offset in degrees Celsius for Robot:2. This value is added to the temperature reported by the sensor on the HPT to calculate the actual temperature near the robot. By default, this is set to 0. This must be set if robot2HumidityOffset is set.
        robot2HumidityOffset:
          type: number
          format: double
          description: Humidity offset in percent relative humidity for Robot:2. This value is added to the humidity reported by the sensor on the HPT to calculate the actual humidity near the robot. By default, this is set to 0. This must be set if robot2TemperatureOffset is set.
    EnvironmentSettings:
      title: Environment Settings
      type: object
      required:
      ...
      properties:
        ...
        robot1TemperatureOffset:
          type: number
          format: double
          description: Temperature offset in degrees Celsius for Robot:1. This value is added to the temperature reported by the sensor on the HPT to calculate the actual temperature near the robot. By default, this is set to 0. If the HPT is replaced, the offset will be automatically adjusted based on the measurements from the other HPT. If no such HPT is present, the offset will be reset to 0 and recalibration will be needed.
        robot2TemperatureOffset:
          type: number
          format: double
          description: Temperature offset in degrees Celsius for Robot:2. This value is added to the temperature reported by the sensor on the HPT to calculate the actual temperature near the robot. By default, this is set to 0. If the HPT is replaced, the offset will be automatically adjusted based on the measurements from the other HPT. If no such HPT is present, the offset will be reset to 0 and recalibration will be needed.
        robot1HumidityOffset:
          type: number
          format: double
          description: Humidity offset in percent relative humidity for Robot:1. This value is added to the humidity reported by the sensor on the HPT to calculate the actual humidity near the robot. By default, this is set to 0. If the HPT is replaced, the offset will be automatically adjusted based on the measurements from the other HPT. If no such HPT is present, the offset will be reset to 0 and recalibration will be needed.
        robot2HumidityOffset:
          type: number
          format: double
          description: Humidity offset in percent relative humidity for Robot:2. This value is added to the humidity reported by the sensor on the HPT to calculate the actual humidity near the robot. By default, this is set to 0. If the HPT is replaced, the offset will be automatically adjusted based on the measurements from the other HPT. If no such HPT is present, the offset will be reset to 0 and recalibration will be needed.
    FRUStatus:
      ...
      discriminator:
        ...
        mapping:
          ...
          FRONT_PANEL: '#/components/schemas/GenericFRUStatus'
      example:
        name: Drive:1:1:1
        type: DRIVE
        status: OK
    ...
    BasicMotionTest:
      ...
      enum:
        ...
        - "VAX_SENSOR"
    DriveTestRequest:
      description: Request to run a Drive Test on a drive
      type: object
      required:
        - driveManufacturerSerialNumber
      properties:
        driveManufacturerSerialNumber:
          ...
          type: string
    DriveTestList:
      description: A list of all Drive Tests that have been run on drives in the library.
      required:
        - count
        - value
      properties:
        ...
        value:
          type: array
          description: List of Drive Test Results
          items:
            $ref: '#/components/schemas/DriveTest'
        nextLink:
          type: string
    ...
    DriveTest:
      description: Result of a Drive Test
      allOf:
        - $ref: '#/components/schemas/Task'
        - type: object
          properties:
      ...
      type: object
      required:
        - spectraDiagnosticCartridgeBarcode
      properties:
        spectraDiagnosticCartridgeBarcode:
          description: Barcode of the Spectra Diagnostic Cartridge that was used for the Drive Test
          type: string
        cleaningTaskID:
          description: Task ID of the cleaning move that was performed prior to the Drive Test
          type: string
        failureData:
          $ref: '#/components/schemas/DriveTestWriteFailure'
    DriveTestWriteFailure:
      description: Data reported by the drive following a failure during the Post-B portion of the test.
      type: object
      properties:
        degradationPercentage:
          description: Percentage of degradation reported by the drive following a Drive Test failure
          type: integer
        degradationDirection:
          $ref: '#/components/schemas/DriveTestDegradationDirection'
        productRevisionLevel:
          description: Product Revision Level of the drive
          type: string
        serviceInformationMessage:
          description: Service Information Message from the diagnostic failure
          type: string
        exceptionMessage:
          description: Exception Message from the diagnostic failure
          type: string
        serviceMessage:
          description: Service Message from the diagnostic failure
          type: string
        severity:
          $ref: '#/components/schemas/DriveTestFailureSeverity'
        interfaceNumber:
          description: Interface that indicated a failure during the diagnostic. This is not defined for every failure
          type: integer
        scdErrorCode:
          description: SCD Error Code from the diagnostic failure
          type: string
    DriveTestDegradationDirection:
      description: Direction of degradation reported by the drive following a Drive Test failure
      type: string
      enum:
        - "Forward"
        - "Backward"
        - "Both Directions"
        - "Unknown"
    DriveTestFailureSeverity:
      description: Severity of the Drive Test failure
      type: string
      enum:
        - "Service"
        - "Moderate"
        ...
        - "Unknown"
    MediaTestRequest:
      description: Request to run a Media Test on a tape
      type: object
      required:
        - tapeSerialNumber
        - driveManufacturerSerialNumber
      properties:
        ...
        driveManufacturerSerialNumber:
          ...
          type: string
    MediaTestList:
        description: A list of Media Tests that have been run on tapes in the library.
        required:
            - count
            - value
        properties:
            ...
            value:
              type: array
              description: List of Media Test Results
              items:
                $ref: '#/components/schemas/MediaTest'
            nextLink:
              type: string
              description: |-
                Link to the next page, omitted if this is the last page
    MediaTest:
      description: Information about a Media Test
      allOf:
        - $ref: '#/components/schemas/Task'
        - type: object
          properties:
            parameters:
              $ref: '#/components/schemas/MediaTestRequest'
    TapeImportList:
      description: Paginated list of tape imports
      required:
        - count
        - value
      properties:
        ...
        value:
          type: array
          description: Tape imports
          items:
            $ref: '#/components/schemas/TapeImport'
        nextLink:
          type: string
          description: Link to the next page, omitted if this is the last page
    TapeImport:
      description: Details about a tape import
      required:
        - barcode
        - serialNumber
        - time
      properties:
        barcode:
          type: string
          description: Barcode of the tape that was imported
        moveTaskID:
          type: string
          description: Task ID of the associated import move
        partition:
          type: string
          description: Name of the partition that the tape was imported into
        serialNumber:
          type: string
          description: Manufacturer serial number of the tape that was imported
        time:
          type: string
          format: date-time
          description: Time when the tape was imported
        user:
          type: string
          description: Name of the user who was logged in when the tape was imported
    TapeExportList:
      description: Paginated list of tape exports
      required:
        - count
        - value
      properties:
        ...
        value:
          type: array
          description: Tape exports
          items:
            $ref: '#/components/schemas/TapeExport'
        nextLink:
          type: string
          description: Link to the next page, omitted if this is the last page
    TapeExport:
      description: Details about a tape export
      required:
        - barcode
        - serialNumber
        - time
      properties:
        barcode:
          type: string
          description: Barcode of the tape that was exported
        moveTaskID:
          type: string
          description: Task ID of the associated export move
        partition:
          type: string
          description: Name of the partition that the tape was exported from
        serialNumber:
          type: string
          description: Manufacturer serial number of the tape that was exported
        time:
          type: string
          format: date-time
          description: Time when the tape was exported
        user:
          type: string
          description: Name of the user who was logged in when the tape was exported
    LEDModes:
      description: |-
    ...
    MediaContainer:
      ...
      properties:
        ...
        sourceAddress:
          description: |-
            The SCSI address of the inventory element where the media was previously located. Note that this address is based on the
            relevant offset in the partition the container is allocated to; two containers in different partitions may have the same address.
          type: integer
          format: int32
          minimum: 1
          maximum: 65535
        ...
        containerState:
          $ref: '#/components/schemas/ContainerStates'
        zone:
          $ref: '#/components/schemas/MediaContainerZone'
    MediaContainerList:
    ...
    MediaContainerAddress:
      ...
      maximum: 65535
    MediaContainerZone:
      description: A location for a media container based on frames. Frames in the left
        half of the library as viewed from the front, are 'MEDIA_ZONE_LEFT'; frames in the right half of the library
        (including the center frame if the number of frames is odd) are in zone 'MEDIA_ZONE_RIGHT'. Zone information is
        only available on TFinity libraries for partitions with "MEDIA_ZONING" configured.
      type: string
      enum:
        - "MEDIA_ZONE_LEFT"
        - "MEDIA_ZONE_RIGHT"
    ...
    MediaMove:
      ...
      example:
        parameters:
          ...
          sourceAddress: 258
          destAddress: 4096
        sourceContainerType: "DRIVE"
        destinationContainerType: "SLOT"
    ...
    Partition:
      ...
      properties:
        ...
        serialNumber:
          description: The serial number of the partition. This is the serial number included in SCSI Inquiry pages
            "Unit Serial Number" (80h) and "Device Identification" (83h) returned by the medium-changer LUN of drives 
            and RIMs. A serial number is not available if the partition does not contain exporting drives or RIMs.
          type: string
          example: "921F003A9E"
    CleaningPartition:
    ...
    EnvironmentSummary:
      ...
      description:
        Temperature and humidity readings from sensors on the library. Available sensor data is dependent on library hardware configuration. If any chassis sensors are present, robot sensors are not needed and will be disabled.
      type: object
      properties:
        chassisTopSensor:
          $ref:
    ...
    MovesSummary:
      ...
      properties:
        ...
        partitionOffsets:
          type: object
          description: A key value map of storage partition names to the slot, EE, and drive address offsets for that partition.
          additionalProperties:
            type: object
            required:
              - slotAddressOffset
            ...
            properties:
              slotAddressOffset:
                type: integer
                description: The SCSI address at which slot addresses begin
                minimum: 1
                maximum: 65535
              eeSlotAddressOffset:
                type: integer
              ...
              driveAddressOffset:
                ...
                maximum: 65535
        partitionDriveLocations:
          description: A key value map of partition names to an array of drive addresses in that partition and their corresponding physical locations.
          type: object
          additionalProperties:
            type: array
            items:
              $ref: '#/components/schemas/DriveLocationSummary'
    RoboticsSummary:
    ...
    Task:
      ...
      properties:
        ...
        tags:
          type: array
          description: Identifying labels for the task
          items:
            type: string
        scheduledStart:
          type: string
    ...
    TimeInterval:
      ...
      example:
        "1h30m2s"
    MLMDLMMountSortOrder:
      type: string
      description: Indicates the sort order of a MLM/DLM mount list.
      enum:
        - "UNLOAD_TIME_ASC"
        - "UNLOAD_TIME_DESC"
    MLMDLMMountTapeUsageCategory:
      type: string
      description: Indicates the tape usage category for records in a MLM/DLM mount list.
      enum:
        - "DATA_STORAGE"
        - "DRIVE_CLEANING"
        - "SPECTRA_DIAGNOSTIC"
    ...
    Subscriber:
      ...
      properties:
        ...
        allowUnencryptedSMTP:
          type: boolean
          description: |-
            Attempt an unencrypted SMTP connection if TLS negotiation fails
        forwardUserMessages:
          type: boolean
          description: |-
            Forward user messages to this subscriber.
        userMessageThreshold:
          $ref: '#/components/schemas/SubscriberUserMessageThreshold'
        companyInfo:
    ...
    DriveLocationSummary:
      ...
      properties:
        driveAddress:
          description: SCSI address of the drive
          type: integer
        physicalLocation:
          description: Physical location of the drive
          $ref: '#/components/schemas/PhysicalLocation'
    SubscriberUserMessageThreshold:
      type: string
      description: |-
        User message severity minimum for generating e-mail alerts. Defaults to "INFO". Ignored if `forwardUserMessages` field is unset or false.
      default: "INFO"
      enum:
        - "INFO"
        - "SUMMARY"
    ...
    CreateSubscriberRequest:
      ...
      properties:
        ...
        allowUnencryptedSMTP:
          type: boolean
          description: |-
            Attempt an unencrypted SMTP connection if TLS negotiation fails. Defaults to false if not specified.
        forwardUserMessages:
          type: boolean
          description: |-
            Forward user messages to this subscriber.
    ...
    GroupNames:
      type: string
      description: |-
        The group to which the user will belong. Note: the "DEGRADED" group is for internal use only;
        attempting to assign a user to this group will result in an error.
      enum:
        - "ADMIN"
        - "OPERATOR"
        - "SUPER_USER"
    ...
    TapeAlertValue:
      enum:
        ...
        - "Cleaning Media"
        - "Unsupported Format"
        - "Recoverable Mechanical Cartridge Fail"
        - "Unrecoverable Mechanical Cartridge Fail"
    ...
    TapeAlert:
      properties:
        value:
          $ref: '#/components/schemas/TapeAlertValue'
    MLMDLMMountList:
      title: MLMDLMMountList - List of MLM/DLM mount records.
      description: |-
        List of Media Lifecycle Management (MLM) Data Lifecycle Management (DLM) mount records.
      required:
        - pagination
        - value
      properties:
        pagination:
          description: Pagination information for the list. Only included when the result set is paginated and not empty.
          type: object
          required:
            - afterCursor
            - afterExists
          ...
          properties:
            afterCursor:
              type: string
              description: Cursor that points to the end of the current page.
            afterExists:
              type: boolean
              description: Indicates if there are more records after the current page.
            beforeCursor:
              type: string
              description: Cursor that points to the start of the current page.
            beforeExists:
              type: boolean
              description: Indicates if there are more records before the current page.
        value:
          type: array
          items:
            $ref: '#/components/schemas/MLMDLMMount'
    MLMDLMMount:
      title: MLMDLMMount - All MLM/DLM data relevant to a tape mount.
      description: |-
        MLMDLMMount - All MLM/DLM data relevant to a tape mount.
      required:
        - tapeSerialNumber
        - driveManufacturerSerialNumber
        - unloadTime
      ...
      properties:
        tapeSerialNumber:
          ...
          type: string
        driveManufacturerSerialNumber:
          description: The manufacturer serial number of the drive.
          type: string
        driveLogicalLocation:
          $ref: '#/components/schemas/LogicalLocation'
        unloadTime:
          description: The time the tape was unloaded from the drive.
          type: string
          format: date-time
        loadTime:
          description: The time the tape was loaded into the drive.
          type: string
          format: date-time
        barcode:
          description: Barcode of the tape cartridge.
          type: string
        cartridgeType:
          $ref: '#/components/schemas/MLMCartridgeTypes'
        manufacturer:
          description: Manufacturer of the tape.
          type: string
        manufactureDate:
          description: Date the tape was manufactured.
          type: string
          format: date-time
        manufacturerID:
          description: |-
        ...
        mamFreeSpaceBytes:
          description: Remaining free space on the Medium Auxiliary Memory (MAM), in bytes.
          type: integer
          format: int64
        totalCapacityMB:
          description: Total capacity of the tape in megabytes.
          type: integer
        usedCapacityMB:
          description: Used capacity of the tape in megabytes.
          type: integer
        remainingCapacityMB:
          description: Remaining capacity of the tape in megabytes.
          type: integer
        writeProtected:
          description: Indicates whether the tape is write-protected.
          type: boolean
        writeCompressionRatio:
          description: Write compression ratio of the tape.
          type: integer
        readCompressionRatio:
          description: Read compression ratio of the tape.
          type: integer
        remainingCleans:
          description: Remaining number of cleaning cycles (only valid for cleaning cartridges).
          type: integer
        driveTestsRemaining:
          description: |-
            The number of Drive Tests remaining on a Spectra Diagnostic Cartridge.
            This field is only applicable for cartridges with a `tapeUsageCategory` of `SPECTRA_DIAGNOSTIC`.
          type: integer
          minimum: 0
        encryption:
          type: object
        ...
        latestPostScan:
          ...
          properties:
            time:
              description: The time of the PostScan verification on the tape.
              type: string
              format: date-time
            result:
              $ref: '#/components/schemas/PostScanResult'
            scanType:
              $ref: '#/components/schemas/PostScanType'
        carbideClean:
          description: |-
          ...
          properties:
            ...
            envHumidity:
              ...
              maximum: 100
        readThisLoad:
          description: Indicates whether the tape was read during this mount.
          type: boolean
        firstWriteLibrary:
          description: The library the tape was in when it was first written to.
          type: string
        firstWritePartition:
          description: The partition the tape was assigned to during the first write.
          type: string
        firstWriteTime:
          description: The timestamp of the first write from the tape.
          type: string
          format: date-time
        mostRecentWriteLibrary:
          description: The library the tape was in when most recently written to.
          type: string
        mostRecentWritePartition:
          description: The partition the tape was assigned to during the most recent write.
          type: string
        mostRecentWriteTime:
          description: The timestamp of the most recent write from the tape.
          type: string
          format: date-time
        firstReadLibrary:
          description: The library the tape was in during the first read.
          type: string
        firstReadPartition:
          description: The partition the tape was assigned to during the first read.
          type: string
        firstReadTime:
          description: The timestamp of the first read from the tape.
          type: string
          format: date-time
        mostRecentReadLibrary:
          description: The library the tape was in during the last read.
          type: string
        mostRecentReadPartition:
          description: The partition the tape was assigned to during the last read.
          type: string
        mostRecentReadTime:
          description: The timestamp of the most recent read from the tape.
          type: string
          format: date-time
        driveDisplay:
          $ref: "#/components/schemas/DisplayMessage"
        lifetimeVolumeMounts:
          description: The number of mounts for the current volume (thread count)
          type: integer
          format: int64
        lifetimeVolumeDatasetsWritten:
          description: The total number of data sets written to the medium in the volume
          type: integer
          format: int64
        lifetimeVolumeRecoveredWriteDataErrors:
          description: The total number of recovered write data correction errors (write retries)
          type: integer
        lifetimeVolumeUnrecoveredWriteDataErrors:
          description: |-
            The total number of times that a write type command was terminated with CHECK CONDITION status
            and a sense key of MEDIUM ERROR or HARDWARE ERROR (unrecovered write errors). Also known as
            'write perms'.
          type: integer
        lifetimeVolumeWriteServoErrors:
          description: |-
            The total number of times that a write was suspended due to detection of a servo condition which could
            result in an incorrectly written track and attempted to write the data at a different location.
            (suspended writes)
          type: integer
        lifetimeVolumeUnrecoveredWriteServoErrors:
          description: |-
            The total number of times that a write was suspended due to detection of a servo condition which could
            result in an incorrectly written track and was unable to write the data at a different location
            (fatal suspended writes)
          type: integer
        lifetimeVolumeDatasetsRead:
          description: The total number of data sets read from the medium in the volume
          type: integer
          format: int64
        lifetimeVolumeRecoveredReadErrors:
          description: The total number of recovered read errors (read retries)
          type: integer
        lifetimeVolumeUnrecoveredReadErrors:
          description: |-
            The total number of times that a read type command was terminated with CHECK CONDITION status
            and a sense key of MEDIUM ERROR or HARDWARE ERROR (unrecovered read errors). Also known as 'read perms'.
          type: integer
        driveHardWriteErrors:
          description: The number of times for this drive that a write type command was terminated with CHECK CONDITION
            status and a sense key of MEDIUM ERROR or HARDWARE ERROR. Note that this count can be reset via a command
            sent directly to the tape drive.
          type: integer
        driveHardReadErrors:
          description: The number of times for this drive that a read type command was terminated with CHECK CONDITION
            status and a sense key of MEDIUM ERROR or HARDWARE ERROR. Note that this count can be reset via a command
            sent directly to the tape drive.
          type: integer
        tapeAlerts:
          description: SCSI TapeAlerts associated with the tape.
          type: array
          items:
            $ref: '#/components/schemas/TapeAlert'
        sessionMaximumTemperature:
        ...
        sessionMinimumHumidity:
          ...
          type: integer
        tapeLifetimeMaximumTemperature:
          description: Maximum temperature in Celsius throughout the lifetime of the tape. This may only be supported for LTO-9 or later and TS-1150 or later generation drives.
          type: integer
        tapeLifetimeMinimumTemperature:
          description: Minimum temperature in Celsius throughout the lifetime of the tape. This may only be supported for LTO-9 or later and TS-1150 or later generation drives.
          type: integer
        tapeLifetimeMaximumHumidity:
          description: Maximum percentage of relative humidity throughout the lifetime of the tape. This may only be supported for LTO-9 or later and TS-1150 or later generation drives.
          type: integer
        tapeLifetimeMinimumHumidity:
          description: Minimum percentage of relative humidity throughout the lifetime of the tape. This may only be supported for LTO-9 or later and TS-1150 or later generation drives.
          type: integer
        tapeHealth:
          description: |-
          ...
          format: double
        libraryPartition:
          description: The name of the library partition which the tape belonged to when the tape was mounted
          type: string
        dutyCycleSampleTimeMS:
          description: The time in milliseconds since the last time the drive was reset. Note that this value can also
            be reset via a command sent directly to the tape drive.
          type: integer
          format: int64
        activityDutyCyclePercentage:
          description: |-
            Percentage of duty cycle sample time that the drive has been active, i.e. processing write
            type commands, read type commands, and other commands that cause the medium to be moved.
          type: integer
          minimum: 0
          maximum: 100
        volumeNotPresentDutyCyclePercentage:
          description: |-
            Percentage of the duty cycle sample time that the drive did not have a volume present
          type: integer
          minimum: 0
          maximum: 100
        sessionMBWritten:
          description: The number of megabytes written during this tape mount after compression
          type: integer
        sessionMBRead:
          description: The number of megabytes read during this tape mount before decompression
          type: integer
        sessionHardWriteErrors:
          description: The number of times during this mount that a write type command was terminated with CHECK CONDITION status and a sense key of MEDIUM ERROR or HARDWARE ERROR (unrecovered write errors). Also known as 'write perms'.
          type: integer
        sessionHardReadErrors:
          description: The number of times during this mount that a read type command was terminated with CHECK CONDITION status and a sense key of MEDIUM ERROR or HARDWARE ERROR (unrecovered read errors). Also known as 'read perms'.
          type: integer
        volumeLifetimeMBWritten:
          description: The total number of megabytes written to this tape over its lifetime after compression
          type: integer
          format: int64
        volumeLifetimeMBRead:
          description: The total number of megabytes read from this tape over its lifetime before decompression
          type: integer
          format: int64
        volumeBeginningOfMediumPasses:
          description: Count of the total number of times the beginning of medium position has passed over the drive head
          type: integer
        volumeMiddleOfMediumPasses:
          description: Count of the total number of times that the physical middle of the user data region on the tape has
            passed over the drive head
          type: integer
        cleanRequested:
          description: Indicates whether the drive requested cleaning after the tape mount
          type: boolean
        timeSinceLastCleaningHours:
          description: The count of hours of tape motion time since the drive was last cleaned
          type: integer
        partition0ApproximateNativeCapacityMB:
          description: The approximate native capacity of partition 0 on this tape in megabytes
          type: integer
        partition0ApproximateUsedNativeCapacityMB:
          description: The approximate used native capacity of partition 0 on this tape in megabytes
          type: integer
        partition0ApproximateRemainingNativeCapacityToEarlyWarningMB:
          description: The approximate remaining native capacity to early warning of partition 0 on this tape in megabytes
          type: integer
        partition1ApproximateNativeCapacityMB:
          description: The approximate native capacity of partition 1 on this tape in megabytes
          type: integer
        partition1ApproximateUsedNativeCapacityMB:
          description: The approximate used native capacity of partition 1 on this tape in megabytes
          type: integer
        partition1ApproximateRemainingNativeCapacityToEarlyWarningMB:
          description: The approximate remaining native capacity to early warning of partition 1 on this tape in megabytes
          type: integer
        firmwareMediumOptimizationVersion:
          description: The version of the Medium Optimization (i.e., TDS calibration) that the currently running firmware uses to optimize new volumes
          type: integer
        lifetimeDriveLoads:
          description: The total number of successful load operations
          type: integer
        lifetimeDriveCleaningOperations:
          description: The total number of successful and failed cleaning operations
          type: integer
        lifetimeDrivePowerOnHours:
          description: The total number of hours the drive has been powered on. The value reported shall be rounded up to the next full hour.
          type: integer
        lifetimeDriveMediumMotionHours:
          description: The total number of hours that the drive has spent processing commands that require medium motion.
            The value reported shall be rounded up to the next full hour.
          type: integer
        lifetimeDriveMetersOfTapeProcessed:
          description: The total number of meters of tape that have been processed by the drive mechanism in either direction
          type: integer
        lifetimeDrivePowerCycles:
          description: The total number of times the drive has detected a power-on event.
          type: integer
        mediumMotionHoursSinceLastSuccessfulClean:
          description: Count of hours that medium has been in motion in either direction since the last successful cleaning operation. The value reported shall be rounded up to the next full hour.
          type: integer
        mediumMotionHoursSinceSecondToLastSuccessfulClean:
          description: Count of hours that medium has been in motion in either direction since the second to last successful cleaning operation. The value reported shall be rounded up to the next full hour.
          type: integer
        mediumMotionHoursSinceThirdToLastSuccessfulClean:
          description: Count of hours that medium has been in motion in either direction since the third to last successful cleaning operation. The value reported shall be rounded up to the next full hour.
          type: integer
        tapeUsageCategory:
          $ref: '#/components/schemas/MLMDLMMountTapeUsageCategory'
        spectraCertifiedMedia:
          description: Indicates whether the tape is Spectra Certified Media
          type: boolean
    MostRecentMLMDLMMountPerDriveList:
      title: MostRecentMLMDLMMountPerDriveList - List of most recent MLM/DLM mount record per drive.
      description: |-
        List of Media Lifecycle Management (MLM) Data Lifecycle Management (DLM) mount records.
      required:
        - count
      ...
      properties:
        ...
        value:
          ...
          items:
            $ref: '#/components/schemas/MLMDLMMount'
    MostRecentMLMDLMMountPerTapeList:
      title: MostRecentMLMDLMMountPerTapeList - List of most recent MLM/DLM mount record per tape.
      description: |-
        List of Media Lifecycle Management (MLM) Data Lifecycle Management (DLM) mount records.
      required:
        - count
    ...
    DLMRecord:
      ...
      properties:
        ...
        temperatureExceededMax:
          ...
          type: boolean
    PostScanResult:
      title: PostScanResult - PostScan Result
      description: The result of the PostScan verification.
      type: string
      enum:
        - "SUCCEEDED"
        - "FAILED"
    PostScanType:
      title: PostScanType - PostScan Type
      description: |-
        Whether the PostScan was a QuickScan or FullScan. A QuickScan only verifies the
        readability of the data on the first wrap, while a FullScan verifies all
        the data on the tape.
    ...
    LibraryTemperatureMetrics:
      ...
      description: Temperature metrics for the library. Available sensor data is dependent on library hardware configuration. If any chassis sensors are present, robot sensors are not needed and will be disabled.
      properties:
        robot1Readings:
          type: array
          items:
    ...
    LibraryHumidityMetrics:
      ...
      description: Humidity metrics for the library. Available sensor data is dependent on library hardware configuration. If any chassis sensors are present, robot sensors are not needed and will be disabled.
      properties:
        robot1Readings:
          type: array
          items:
